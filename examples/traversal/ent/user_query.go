...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withManage; query != nil {
		...
		for _, n := range neighbors {
			fk := n.GroupAdmin
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "GroupAdmin" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "GroupAdmin" returned %v for node %v`, *fk, n.ID)
			}
