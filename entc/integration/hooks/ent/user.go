...
func (u *User) assignValues(values ...interface{}) error {
	...
	if len(values) == len(user.ForeignKeys) {
		if value, ok := values[0].(*sql.NullInt64); !ok {
			return fmt.Errorf("unexpected type %T for edge-field UserBestFriend", value)
		} else if value.Valid {
			u.UserBestFriend = new(int)
			*u.UserBestFriend = int(value.Int64)
		}
	}
	return nil
}
